...
(function(root, factory) {
	...
	_.extend(Collection.prototype, Events, {
		...
		set : function(models, options) {
			...
			for (i = 0, l = models.length; i < l; i++) {
				...
				if (existing = this.get(model)) {
					...
					// Listen to added models' events, and index models for
					// lookup by
					// `id` and by `cid`.
		...
		// Figure out the smallest index at which a model should be inserted so
		// as
		// to maintain order.
	...
	var methods = [ 'forEach', 'each', 'map', 'collect', 'reduce', 'foldl', 'inject', 'reduceRight', 'foldr', 'find', 'detect', 'filter', 'select', 'reject', 'every', 'all', 'some', 'any', 'include', 'contains', 'invoke', 'max', 'min', 'toArray', 'size', 'first', 'head', 'take',
			'initial', 'rest', 'tail', 'drop', 'last', 'without', 'indexOf', 'shuffle', 'lastIndexOf', 'isEmpty', 'chain' ];
